...
function fetchImages() {
  fetch(db.json)
    ...
    .then(images => {
      shibaImages = images // images.map((item) => item.imageUrl);
      displayImagesWithTags();
    })
    .catch(error => {
...
function displayImagesWithTags() {
  imageContainer.innerHTML = '';
  shibaImages.slice(0, 9).forEach((imageData, index) => {
    const { imageUrl, tag } = imageData;
    ...
    imageWrapper.append(tagDiv);
    imageWrapper.append(imageWrapper);

    imageWrapper.addEventListener('mouseenter', () => highlightTag(tagDiv, tag));
    imageWrapper.addEventListener('mouseleave', () => removeHighlight(tagDiv));
    imageWrapper.addEventListener('click', () => likeImage(imageWrapper))
  })
}

function highlighTag(tagDiv, tag) {
  tagDiv.classList.add('highlighted');
  searchForTag(tag);
}

function removeHighlight(tagDiv) {
  tagDiv.classList.remove('highlighted');
}

function likeImage(imageWrapper) {
  likeButton.className = 'like-button';
  likeButton.innerHTML = `Like <span class="like-glyph">&#x2661;</span>`;
  imageWrapper.append(likeButton);

  likeButton.addEventListener('click', (e) => {
    e.stopPropagation();
    likeButton.classList.add('activated-heart')
    likeButton.innerHTML = `Like! <span class="like-glyph">${FULL_HEART}</span>`;
  })
}
...
function searchForTag(tag) {
  const matchedImageURL = document.querySelectorAll('.tag.highlighted');
  matchedImages.forEach((image) => {
    if (image.textContent.toLowerCase() !== tag.toLowerCase()) {
      removeHighlight(image);
    }
  });
}
  // shibaImages.find((imageURL) =>
...
//   if (shibaImages.length > 0) {
//     if (currentImageIndex < shibaImages.length && currentImageIndex < 9) {
//       const imageUrl = shibaImages[currentImageIndex];
